...
## 关于系统调用详细讲解
[https://blog.csdn.net/fang92/article/details/48418127](https://blog.csdn.net/fang92/article/details/48418127)
...
```
而在kern/syscall.c中可以得知，syscall的第一个参数是系统调用的号码
```
int32_t
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
```
也就是说syscall将系统调用的号码存在了eax寄存器中，并且返回。